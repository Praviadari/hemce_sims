  ...
  const startScan = () => {
    ...
    const tick = (now) => {
      ...
      if (p < 1) animRef.current = requestAnimationFrame(tick);
        ...
        setScanning(false);
        setResult(s.det ? "DETECTED" : "CLEAR");
      }
    };
...
Part 3 — INDIA-SPECIFIC CONTEXT: How does this relate to DRDO/HEMRL programs? Reference specific Indian systems (e.g., Agni, BrahMos, Pinaka, SMART, Astra, Nag, Akash) where applicable. What are India's current capabilities and gaps in this domain?`,
  ...
  return (
    <div>
      ...
      <PillRow>
        <Pill
          active={method === "colorimetric"}
          onClick={() => {
            setResult(null);
            setMethod("colorimetric");
          }}
          color={T.orange}
        >
          Colorimetric
        ...
        <Pill
          active={method === "spectroscopic"}
          onClick={() => {
            setResult(null);
            setMethod("spectroscopic");
          }}
          color={T.purple}
        >
          Raman
      ...
      <InfoBox>
        <strong style={{ color: T.purple }}>Detection:</strong>{" "}
        {method === "colorimetric"
          ? "HEMRL field kit — color reagents, used by police/BSF."
          : method === "spectroscopic"
            ? "Raman — laser molecular fingerprinting, ppb sensitivity."
            : method === "ion_mobility"
              ? "IMS — airport-grade screening, low false-positive."
              : "Canine — ppt sensitivity, gold standard for field sweeps."}
      </InfoBox>
      <AIInsight buildPrompt={buildPrompt} color={T.pink} />
      <ExportBtn
        simId="explosive_detection"
        getData={() => ({ method, sample, scanning, result, scanProg, sensitivity: samples[sample].sensitivity })}
        color={T.pink}
      />
    </div>
  );
