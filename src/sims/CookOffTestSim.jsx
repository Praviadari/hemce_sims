...
export function CookOffTestSim() {
  ...
  return (
    <div>
      ...
      <DataRow>
        <DataBox label="Cook-off Temp" value={munType === "insensitive" ? "180+" : "140"} unit="°C" color={munType === "insensitive" ? "#3b82f6" : undefined} />
        <DataBox label="Reaction Type" value={munType === "insensitive" ? "Burn" : "Detonate"} color={munType === "insensitive" ? "#3b82f6" : undefined} />
      </DataRow>
      ...
      <InfoBox color={munType === "insensitive" ? "#3b82f6" : T.accent}>
        Insensitive Munitions (IM) are designed to withstand catastrophic external stimuli (Fast Cook-off, Bullet Impact, Sympathetic Detonation) using advanced polymer-bonded explosives (PBX) and venting casings. They simply burn instead of detonating violently, saving vessels and crews from catastrophic chain reactions.
      </InfoBox>

      <AIInsight buildPrompt={() => `Explain the strategic importance of STANAG 4439 compliant Insensitive Munitions (IM) compared to standard TNT/RDX equivalents in modern defense platforms.`} color={munType === "insensitive" ? "#3b82f6" : undefined} />
    </div>
  );
}
