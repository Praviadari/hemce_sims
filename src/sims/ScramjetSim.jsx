...
export default function ScramjetSim() {
  ...
  const canvasRef = useCanvas(
    (ctx, W, H, frame) => {
      ...
      ctx.beginPath();
      ctx.moveTo(340, cy - 40);
      ctx.lineTo(410, cy - 60);
      ctx.lineTo(410, cy + 60);
      ctx.lineTo(340, cy + 40);
      ctx.fill();
      ...
      ctx.fillStyle = T.cyan;
      ctx.fillText("SUPERSONIC INLET", 60, cy + 55);
      ctx.fillStyle = T.gray;
      ctx.fillText("ISOLATOR", 115, cy + 55);
      ctx.fillStyle = T.orange;
      ctx.fillText("SCRAM-COMBUSTOR", 200, cy + 55);
      ctx.fillStyle = T.accent;
      ctx.fillText("EXPANSION NOZZLE", 310, cy + 55);
      ctx.textAlign = "left";
...
Related Indian systems: ${related.map((m) => m.name).join(", ")}`,
  ...
  return (
    <div>
      ...
      <InfoBox>
        <strong style={{ color: T.cyan }}>Scramjet:</strong> Air-breathing, supersonic combustion at Mach 5+. Uses
        atmospheric O₂.{" "}
        {cooling
