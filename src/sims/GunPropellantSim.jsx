...
export default function GunPropellantSim() {
  ...
  const sd = {
    single: { prog: "degressive", peak: 280 },
    "7perf": { prog: "neutral", peak: 350 },
    "19perf": { prog: "progressive", peak: 420 },
  }[grainShape];
  ...
  const canvasRef = useCanvas(
    (ctx, W, H, frame) => {
      ...
      if (running && p > 0.85 && p < 1) {
        ...
        ctx.fillStyle = fGrad;
        ctx.beginPath();
        ctx.arc(W - 20, cy, 60 * flashP, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 10; i++) {
          ...
          ctx.beginPath();
          ctx.arc(
            W - 20 + prng(frame, i * 2) * 40 * flashP,
            cy + (prng(frame, i * 2 + 1) - 0.5) * 20,
            1,
            0,
            Math.PI * 2,
          );
...
Part 3 — INDIA-SPECIFIC CONTEXT: How does this relate to DRDO/HEMRL programs? Reference specific Indian systems (e.g., Agni, BrahMos, Pinaka, SMART, Astra, Nag, Akash) where applicable. What are India's current capabilities and gaps in this domain?`,
  ...
  return (
    <div>
      ...
      <PillRow>
        <Pill
          active={grainShape === "single"}
          onClick={() => {
            reset();
            setGrainShape("single");
          }}
          color={T.orange}
        >
          Single-Perf
        ...
        <Pill
          active={grainShape === "19perf"}
          onClick={() => {
            reset();
            setGrainShape("19perf");
          }}
          color={T.green}
        >
          19-Perf
      ...
      <InfoBox>
        <strong style={{ color: T.gold }}>Interior ballistics:</strong> Grain perforation count controls burn
