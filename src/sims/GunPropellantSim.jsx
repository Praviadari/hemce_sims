...
export default function GunPropellantSim() {
  ...
  const canvasRef = useCanvas((ctx, W, H) => {
    ...
    ctx.fillStyle = "#2A2A2A"; ctx.fillRect(30, cy - 18, W - 50, 36); ctx.strokeStyle = "#555"; ctx.lineWidth = 2; ctx.strokeRect(30, cy - 18, W - 50, 36);
    ctx.fillStyle = "#1A1A1A"; ctx.fillRect(30, cy - 22, 60, 44);
    if (p < 0.3) { for (let i = 0; i < 8; i++) { ctx.fillStyle = "#8B7355"; ctx.beginPath(); ctx.arc(40 + i * 6, cy - 8 + (i % 3) * 8, 3, 0, Math.PI * 2); ctx.fill(); } }
    if (running && p > 0 && p < 0.8) { const gl = p * 200; const g = ctx.createLinearGradient(90, 0, 90 + gl, 0); g.addColorStop(0, "rgba(255,100,0,.4)"); g.addColorStop(1, "rgba(255,200,0,0)"); ctx.fillStyle = g; ctx.fillRect(90, cy - 14, gl, 28); }
    const px = 90 + p * (W - 140); ctx.fillStyle = "#888"; ctx.beginPath(); ctx.moveTo(px + 15, cy); ctx.lineTo(px, cy - 10); ctx.lineTo(px - 5, cy - 10); ctx.lineTo(px - 5, cy + 10); ctx.lineTo(px, cy + 10); ctx.closePath(); ctx.fill();
    if (running && p > 0.7 && p < 1) { for (let i = 0; i < 6; i++) { const a = (Math.random() - 0.5) * 1.2, d = 10 + Math.random() * 40; ctx.strokeStyle = `rgba(255,220,100,${0.4 + Math.random() * 0.3})`; ctx.lineWidth = 1 + Math.random() * 2; ctx.beginPath(); ctx.moveTo(W - 20, cy); ctx.lineTo(W - 20 + Math.cos(a) * d, cy + Math.sin(a) * d); ctx.stroke(); } }
