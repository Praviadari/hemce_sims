...
export default function AdditiveManufacturingSim() {
  ...
  const pd = {
    fdm: { n: "FDM", sp: 40, res: 0.2 },
    sls: { n: "SLS", sp: 25, res: 0.1 },
    dmls: { n: "DMLS", sp: 10, res: 0.04 },
    dw: { n: "Direct Write", sp: 5, res: 0.5 },
  }[process];
  ...
  const canvasRef = useCanvas(
    (ctx, W, H, frame) => {
      ...
      for (let i = 0; i < layer; i++) {
        ...
        let baseCol;
        if (process === "dmls")
          baseCol = 200; // Blueish metal
        else if (process === "dw")
          baseCol = 30; // Orange energetic
      ...
      if (printing && layer < totalLayers) {
        ...
        ctx.fillStyle = fGrad;
        ctx.beginPath();
        ctx.arc(bx + bw / 2 + moveX, hy, 8, 0, Math.PI * 2);
        ctx.fill();
...
Part 3 — INDIA-SPECIFIC CONTEXT: How does this relate to DRDO/HEMRL programs? Reference specific Indian systems (e.g., Agni, BrahMos, Pinaka, SMART, Astra, Nag, Akash) where applicable. What are India's current capabilities and gaps in this domain?`,
  ...
  return (
    <div>
      ...
      <InfoBox>
        <strong style={{ color: T.accent }}>AM for HEM:</strong>{" "}
        {process === "dmls"
          ? "DMLS: Ti-6Al-4V nozzles with complex cooling channels."
          : process === "dw"
            ? "Direct-Write: Energetic paste extrusion for custom grains."
            : process === "sls"
              ? "SLS: Sintered nylon/metal for rapid prototyping."
              : "FDM: ABS/PEEK fuel grains with tailored ports."}{" "}
        DRDO actively adopting AM.
      </InfoBox>
      <AIInsight buildPrompt={buildPrompt} color={T.lime} />
      <ExportBtn
        simId="additive_manufacturing"
        getData={() => ({ process, part, infill, totalLayers, buildTime, partDensity, printing })}
        color={T.lime}
      />
    </div>
  );
